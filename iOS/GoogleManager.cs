...
{
    public class GoogleManager : NSObject, IGoogleLoginInterface, ISignInDelegate, ISignInUIDelegate
    {
        private Action<User, string> _onLoginComplete;
        ...
        public void DidSignIn(SignIn signIn, Google.SignIn.GoogleUser user, NSError error)
        {
            ...
            if (user != null && error == null)
                ...
                _onLoginComplete?.Invoke(new User()
                {
                   ...
                   Picture = user.Profile.HasImage ? user.Profile.GetImageUrl(500).ToString() : string.Empty
                }, string.Empty);
            else
